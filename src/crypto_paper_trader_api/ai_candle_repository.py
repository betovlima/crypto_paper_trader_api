...
class AICandleRepository:
    def upsert_frame(self, session: Session, market: str, timeframe: str, frame: pd.DataFrame) -> int:
        ...
        for row in frame.itertuples(index=False):
            ...
            rows.append({
                "market": market, "timeframe": timeframe, "candle_timestamp": ts,
                "open": float(row.open), "high": float(row.high), "low": float(row.low),
                "close": float(row.close), "volume": float(row.volume),
                "value": float(getattr(row, "value", 0.0)), "inserted_at": now,
            })
        ...
        for offset in range(0, len(rows), 80):
            ...
            stmt = stmt.on_conflict_do_update(
                ...
                set_={
                    ...
                    "volume": stmt.excluded.volume, "value": stmt.excluded.value,
                },
            )
    ...
    def load_frame(self, session: Session, market: str, timeframe: str, limit: int) -> pd.DataFrame:
        rows = session.execute(
            ...
            .where(AIMarketCandle.market == market, AIMarketCandle.timeframe == timeframe)
        ...
        return pd.DataFrame([
            {
                "market": row.market,
                "timestamp": pd.Timestamp(row.candle_timestamp, tz="UTC") if row.candle_timestamp.tzinfo is None else pd.Timestamp(row.candle_timestamp),
                "open": row.open,
                "high": row.high,
                "low": row.low,
                "close": row.close,
    ...
    def coverage(self, session: Session, market: str, timeframe: str) -> dict[str, object]:
        ...
        return {"stored_candles": int(count or 0), "first_candle_at": first_ts, "last_candle_at": last_ts}

    def save_state(self, session: Session, market: str, timeframe: str, target: int, status: str, missing: int = 0, error: str | None = None) -> AIHistorySyncState:
        coverage = self.coverage(session, market, timeframe)
        state = session.scalar(select(AIHistorySyncState).where(AIHistorySyncState.market == market, AIHistorySyncState.timeframe == timeframe))
        if state is None:
            state = AIHistorySyncState(market=market, timeframe=timeframe, target_candles=target, updated_at=datetime.now(timezone.utc))
            session.add(state)
