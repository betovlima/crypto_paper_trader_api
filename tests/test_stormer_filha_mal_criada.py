...
def row(close=110.0, high=111.0, low=104.5, timestamp="2026-07-21T12:00:00Z", ema_shift=0.0):
    values = {
        "timestamp": pd.Timestamp(timestamp), "open": 109.0, "high": high, "low": low, "close": close,
...
def test_triggers_buy_when_later_candle_breaks_armed_high():
    ...
    later = row(high=trigger + 0.5, low=108.5, timestamp="2026-07-21T12:30:00Z")
